...
import { Search, Stop, Trash2 } from "lucide-react";
import { Card } from "@/components/ui/card";
import { Input } from "@/components/ui/input";
import { Button } from "@/components/ui/button";
import { ScrollArea } from "@/components/ui/scroll-area";
import { useToast } from "@/hooks/use-toast";
import { stopShare, deleteShare, type ShareProcess } from "@/lib/api";
...
export function ProcessList({ processes }: ProcessListProps) {
  ...
  // Group processes by post link
  const processesByPostLink = filteredProcesses.reduce((acc, [id, process]) => {
    ...
    return acc;
  }, {} as Record<string, { processes: Array<{ id: string } & ShareProcess>, totalShares: number, completedShares: number }>);
